    ...
    for (const object::SectionRef &Section: Obj.sections()) {
        ...
        if (!Section.getName(sName) && sName == ".gnu_debuglink") {
            ...
            if (!Section.getContents(Contents)) {
                ...
                info.crc32 = *(const uint32_t*)Contents.substr(LLT_ALIGN(length, 4), 4).data();
                break;
            }
        }
    }
...
// GOAL: Assign errorobj
            ...
            *obj = (llvm::object::MachOObjectFile *)origerrorobj;
            ...
            std::string dsympath = fname;
            dsympath += ".dSYM/Contents/Resources/DWARF/";
            dsympath += fname.substr(sep + 1);
#ifdef LLVM35
...
// GOAL: Assign *obj, *context, *slide (if above succeeded)
            if (errorobj) {
                ...
                *obj = binary.first.release();
                ...
                // for now, we only support files next to the binary;
                // in the future, we may also search in .debug and/or user-specified locations
                debug_link_info info = getDebuglink(**obj);
                if (!info.filename.empty()) {
                    size_t sep = fname.rfind('/');
                    std::string debuginfopath = fname.substr(0, sep + 1);
                    debuginfopath += info.filename;
                    ErrorOr<std::unique_ptr<MemoryBuffer>> SplitFile =
                        MemoryBuffer::getFile(debuginfopath);
                    if (SplitFile) {
                        uint32_t crc32 = calc_gnu_debuglink_crc32(
                                SplitFile.get()->getBufferStart(),
                                SplitFile.get()->getBufferSize());
                        if (crc32 == info.crc32) {
                            auto error_splitobj = llvm::object::ObjectFile::createObjectFile(
                                    SplitFile.get().get()->getMemBufferRef(),
                                    sys::fs::file_magic::unknown);
                            if (error_splitobj) {
                                // successfully validated and loaded split debug info,
                                // use that instead of the obj originally loaded
                                delete *obj;
                                *obj = error_splitobj.get().release();
                                SplitFile.get().release();
                            }
#ifdef LLVM39
                            else {
                                consumeError(error_splitobj.takeError());
                            }
#endif
                        }
                    }
                        ...
                        consumeError(SplitFile.takeError());
                    }
#endif
                }
#endif
